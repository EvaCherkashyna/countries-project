...
export const Info = (props) => {
    ...
    const {
        ...
        navigate } = props;
    useEffect(() => {
     if(borders.length>0){axios.get(filterByCode(borders)).then(({ data }) => setNeighbors(data.map(country => country.name)))}
    }, [borders])
    return (
        <Wrapper>
            ...
            <div>
                ...
                <ListGroup>
                    ...
                    <List>
                        ...
                        <ListItem>
                            <b>Currency </b>
                            {currencies.map((current) => (<span key={current.code}>{current.name}</span>))}
                        </ListItem>
                        <ListItem>
                            <b>Language </b>
                ...
                <Meta>
                    ...
                    {
                        neighbors.length == 0 ? (<span>There is no border countries</span>) :
                            (
                                <TagGroup>
                                    {neighbors.map((border) =>
                                        (<Tag key={border} onClick={() => navigate(`country/${border.toLowerCase()}`)}>{border}</Tag>)
                                    )}
                                </TagGroup>
                            )
                    }
